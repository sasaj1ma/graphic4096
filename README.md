...
- `src/jp-strip.js` — 日本語の1行を、横に流すための細長いビットマップに焼く。`message board` と、その実機用データの焼き出しが使う
...
- `message board` — 新幹線の車内案内のように、ことばが右から左へ流れる電光掲示板。漢字は紫、かなは白っぽく光る。先頭の `SETTINGS` に流したいことばを書く（日本語フォントから焼くので、字数も文字も自由）。実機用のデータは `work/bake-message-board.html` で焼き直す
